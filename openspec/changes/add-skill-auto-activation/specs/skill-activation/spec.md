# Spec: Skill Activation

**Capability:** Auto-activate skills based on user prompts, keywords, and intent patterns

**Status:** ADDED (new capability)

## Overview

The skill activation system automatically suggests relevant skills when users submit prompts that match predefined keywords or intent patterns. This replaces the unreliable natural language-based skill discovery with structured, consistent activation rules.
...
- **Skill Discovery:** Current natural language-based discovery (replaced by this)
- **Skill Loading:** Existing skill file loading (unchanged)
- **Hook System:** Claude Code's hook infrastructure (utilized)

## Non-Functional Requirements

### NFR-001: Maintainability

The hook SHALL be maintainable with TypeScript and strict typing.

- Use TypeScript with strict mode
- Document all functions with JSDoc
- Keep hook logic under 300 lines

### NFR-002: Extensibility

The architecture SHALL support future enhancements.

- Design supports file-based triggers (future)
- Schema allows additional enforcement types (future)
- Hook architecture supports additional matching strategies (future)

### NFR-003: User Experience

The system SHALL provide clear feedback and require minimal configuration.

- Clear error messages with actionable steps
- Zero-config default (works out of box)
- Fast execution (<50ms) with no perceived latency

### NFR-004: Compatibility

The system SHALL work with existing Claude Code infrastructure.

- Compatible with Claude Code CLI v1.x
- Requires Bun runtime (documented prerequisite)
- Compatible with all existing plugins

## Dependencies

- **Bun:** Runtime for TypeScript hook execution
- **Claude Code Hooks:** UserPromptSubmit hook support
- **Node.js fs/path:** File system operations (built-in)

## Future Enhancements (Not in MVP)

- **File-based triggers:** Activate based on edited files
- **Guardrail enforcement:** Block actions until skill used
- **Skill dependencies:** "Requires X, suggests Y"
- **Activation analytics:** Track which triggers work best
- **Context-aware priorities:** Adjust priority based on context
